...
export default definePlugin({
  ...
  async run(ctx) {
    ...
    for (const branch of branches) {
      const res = await fetch(`https://api.github.com/repos/${ctx.inputs.sourceRepo}/git/refs/heads/${branch}`, { headers });
      if (!res.ok) { results.push({ branch, success: false }); continue; }
      const data = await res.json() as { object: { sha: string } };
      const pushRes = await fetch(`https://api.github.com/repos/${ctx.inputs.targetRepo}/git/refs/heads/${branch}`, { method: "PATCH", headers, body: JSON.stringify({ sha: data.object.sha }) });
